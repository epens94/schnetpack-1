...
class AdaptiveLossFunction(torch.nn.Module):
    ...
    def __init__(
        ...
        if alpha_init is None:
            self.register_buffer(
                "alpha_init", torch.tensor((self.alpha_lo + self.alpha_hi) / 2.0)
            )
        else:
    ...
    def lossfun(self, x, **kwargs):
        ...
        #
        _alpha = self.alpha_in_init(
            self.latent_alpha, alpha_lo=self.alpha_lo, alpha_hi=self.alpha_hi
        )
        _scale = self.scale_in_init(
            self.latent_scale, lo=self.scale_lo, ref=self.scale_init
        )
